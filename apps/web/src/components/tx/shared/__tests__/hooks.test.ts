...
describe('SignOrExecute hooks', () => {
  ...
  describe('useTxActions', () => {
    ...
    it('should sign a not fully signed tx before executing with a hardware wallet', async () => {
      jest.spyOn(walletHooks, 'isSmartContractWallet').mockReturnValue(Promise.resolve(false))

      // Connected wallet is a Ledger
      jest.spyOn(wallet, 'default').mockReturnValue({
        chainId: '1',
        label: 'Ledger',
        address: '0x1234567890000000000000000000000000000000',
      ...
      // Not relayed (isRelayed omitted), tx has 1/2 signatures
      const id = await executeTx({ gasPrice: 1 }, tx, '123')
      expect(signSpy).toHaveBeenCalled()
      expect(executeSpy).toHaveBeenCalled()
      // Re-proposed after collecting the extra signature
      expect(proposeSpy).toHaveBeenCalled()
      expect(id).toEqual('123')
    })

    it('should not request an extra signature when executing with a hardware wallet that is already fully signed', async () => {
      jest.spyOn(walletHooks, 'isSmartContractWallet').mockReturnValue(Promise.resolve(false))
      ...
      const signSpy = jest.spyOn(txSender, 'dispatchTxSigning').mockImplementation(() => Promise.resolve(tx))
      const executeSpy = jest
        .spyOn(txSender, 'dispatchTxExecution')
        .mockImplementation((() => Promise.resolve(createSafeTx())) as unknown as typeof txSender.dispatchTxExecution)
      ...
      // 1/1 signatures: already meets threshold, no extra signing needed
      const id = await executeTx({ gasPrice: 1 }, tx, '123')
